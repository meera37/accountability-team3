    ...
    <header className="sg__header sticky top-0 bg-white shadow-md z-50">
      <div className="sg__container flex flex-wrap items-center justify-between w-full max-w-screen-xl mx-auto px-4 py-3">
        ...
        <div className="logo flex items-center gap-3 w-full sm:w-auto">
          ...
          <h1 className="text-xl font-semibold">Accebility</h1>
        </div>
        ...
        {/* Navigation Menu */}
        <nav
          className={`navigation w-full lg:w-auto bg-white lg:bg-transparent transition-all duration-300 ease-in-out ${
            menuopen ? 'flex flex-col absolute top-full left-0 right-0 px-4 py-2 shadow-md' : 'hidden'
        ...
        >
          {Object.keys(header_links).map((text, idx) => (
            <Link
              ...
              key={`navigation${idx}`}
              className="capitalize px-3 py-2 text-gray-700 hover:text-blue-600 nav-item block"
